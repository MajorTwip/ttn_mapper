...
Logger logger;  Logger::LEVEL loglevel = Logger::LEVEL::DEBUG;
...
void setup(){
    ...
    String tmp; Logger::hexArray2Str(otaa_creds.deveui,8u,tmp);
    logger.debug(String("deveui: ") + tmp);
    tmp.clear(); Logger::hexArray2Str(otaa_creds.appeui,8u,tmp);
    logger.debug(String("appeui: ") + tmp);
    tmp.clear(); Logger::hexArray2Str(otaa_creds.appkey,16u,tmp);
    ...
    lorawan.RegisterListener(
        ...
        [](void *pClientInfo, uint32_t event) -> void {
            ...
            switch (event)
            {
            case EV_JOINING: info_LORA.update("JOINING");break;
            case EV_JOIN_TXCOMPLETE: info_LORA.update("JOIN SENT");break;
            case EV_JOINED: info_LORA.update("JOINED OK");break;
            case EV_JOIN_FAILED: info_LORA.update("JOIN FAIL");break;

            case EV_TXSTART:info_LORA.update("TX START");break;
            case EV_TXCOMPLETE:info_LORA.update("TX OK");break;

            case EV_RXSTART:info_LORA.update("RX START");break;
            case EV_RXCOMPLETE:info_LORA.update("RX OK");break;
